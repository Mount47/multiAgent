"""Workflow execution tracer - tracks state transitions, timing, and token usage."""
...
class WorkflowTracer:
    ...
    def exit_state(self, task_id: str, estimated_tokens: int = 0) -> None:
        self._close_current_state(task_id, estimated_tokens)
    ...
    def end_workflow(self, task_id: str, status: str = "completed") -> WorkflowTrace | None:
        ...
        if trace:
            ...
            trace.total_tokens = sum(t.estimated_tokens for t in trace.transitions)
            logger.info(
                "[trace] workflow ended: %s status=%s states=%d tokens=%d duration=%.0fms",
                task_id[:8], status, trace.state_count, trace.total_tokens, trace.duration_ms,
            )
        return trace
    ...
    def _close_current_state(self, task_id: str, estimated_tokens: int = 0) -> None:
        ...
        trace = self._traces.get(task_id)
        if trace:
            # Patch previous transition's to_state
